...
| API Gateway in front of Lambda, API-key auth | Unlike HC's Reference Project (plain Lambda Function URL), this project puts API Gateway in front of the Command Centre Lambda specifically as the auth layer — an API key per caller, no SigV4 needed from the C client. Implemented as REST APIs (`aws_api_gateway_rest_api`, not the v2 HTTP API — only v1 supports native API-key/usage-plan auth). The hub's write and read paths get one REST API + key each, for genuine per-key route isolation; the Android app's two routes share a third REST API + key instead, since both belong to the same trusted caller (`apigateway.tf`, §4). |
