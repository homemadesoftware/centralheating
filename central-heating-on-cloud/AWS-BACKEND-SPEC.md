...
| `lambda.tf` | Command Centre Lambda — status ingest (API Gateway-triggered, write key); a second, separate Lambda that mints a presigned S3 GET URL for the desired-state object (API Gateway-triggered, read key); possibly a third for writing desired state (or that could just be a manual `aws s3 cp` / small script, given how infrequently it changes) |
...
| `iam.tf` | Roles for: Command Centre Lambda (write to desired-state bucket's companion "last-seen status" location, if kept; read secrets bucket), mint-URL Lambda (needs `s3:GetObject`-presign permission on exactly the desired-state object, nothing broader) |
...
| Cost | Expected near-zero at this volume — API Gateway, Lambda, S3, and ECR storage all sit within or close to AWS's always-free tier for a single-device, low-frequency workload. Not a factor in any decision above. |

### Still open

| Question | Notes |
|---|---|
| Where does "current status" live, if anywhere? | QUACK's heartbeat is fire-and-forget from the Pico's perspective; this spec assumes the Command Centre Lambda at minimum logs/stores the latest snapshot somewhere (S3 alongside desired-state, or just CloudWatch Logs) so there's something to look at, but "build a dashboard" is explicitly out of scope for this round. |
