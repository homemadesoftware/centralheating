...
#
# The integration/permission/deployment/stage/usage-plan resources below are
# TEMPORARILY COMMENTED OUT (2026-08-30) — they depend, directly or via
# invoke_arn/function_name, on the two aws_lambda_function resources in
# lambda.tf, which are themselves commented out until a real image exists in
# ECR (see lambda.tf). Line-commented rather than /* */ because the ARN
# wildcard pattern "/*/*"" below contains a literal "*/" that would close a
# block comment early. The rest_api/resource/method/api_key resources don't
# depend on the functions and already exist — left active so `tofu apply`
# doesn't try to destroy them. Uncomment everything here together with
# lambda.tf once an image has been pushed.
