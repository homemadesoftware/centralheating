...
_InitialiseLcd:
	...
	ar1 = 0x01
	ar0 = 0x00
;	LcdDisplay.c:55: PORT_LCD_E = 0;						// Start LCD Control (Disable)
;	assignBit
	clr	_P0_1
;	LcdDisplay.c:56: PORT_LCD_RS = 0;						// Default Instruction
;	assignBit
	clr	_P0_3
;	LcdDisplay.c:57: PORT_LCD_RW = 0;						// Default = Write Direction
;	assignBit
	clr	_P0_2
;	LcdDisplay.c:58: for (i=0;i<10000;i++);				// Power-On Delay (15 mS)  
	mov	r6,#0x10
	mov	r7,#0x27
00112$:
	dec	r6
...
00164$:
	...
	orl	a,r7
	jnz	00112$
;	LcdDisplay.c:60: PORT_LCD &= 0x0F;       				// Clear old LCD Data (Bit[7..4])
	anl	_P0,#0x0f
;	LcdDisplay.c:61: PORT_LCD |= 0x30;						// DB5:DB4 = 1:1
	orl	_P0,#0x30
;	LcdDisplay.c:62: PulseLcdEnable();							// Enable Pulse
	lcall	_PulseLcdEnable
;	LcdDisplay.c:63: for (i=0;i<2500;i++);					// Delay 4.1mS
	mov	r6,#0xc4
	mov	r7,#0x09
00115$:
	dec	r6
...
00166$:
	...
	orl	a,r7
	jnz	00115$
;	LcdDisplay.c:65: PORT_LCD &= 0x0F;       				// Clear old LCD Data (Bit[7..4])
	anl	_P0,#0x0f
;	LcdDisplay.c:66: PORT_LCD |= 0x30;						// DB5:DB4 = 1:1
	orl	_P0,#0x30
;	LcdDisplay.c:67: PulseLcdEnable();							// Enable Pulse
	lcall	_PulseLcdEnable
;	LcdDisplay.c:68: for (i=0;i<100;i++);					// delay 100uS
	mov	r6,#0x64
	mov	r7,#0x00
00118$:
	dec	r6
...
00168$:
	...
	orl	a,r7
	jnz	00118$
;	LcdDisplay.c:70: PORT_LCD &= 0x0F;       				// Clear old LCD Data (Bit[7..4])
	anl	_P0,#0x0f
;	LcdDisplay.c:71: PORT_LCD |= 0x30;						// DB5:DB4 = 1:1
	orl	_P0,#0x30
;	LcdDisplay.c:72: PulseLcdEnable();							// Enable Pulse
	lcall	_PulseLcdEnable
...
;	LcdDisplay.c:75: PORT_LCD &= 0x0F;       				// Clear old LCD Data (Bit[7..4])
	anl	_P0,#0x0f
;	LcdDisplay.c:76: PORT_LCD |= 0x20;						// DB5:DB4 = 1:0
	orl	_P0,#0x20
;	LcdDisplay.c:77: PulseLcdEnable();							// Enable Pulse
	lcall	_PulseLcdEnable
...
;	assignBit
	clr	_P0_3
;	LcdDisplay.c:96: PORT_LCD_RW  = 0;						// Write Select
;	assignBit
	clr	_P0_2
...
;	LcdDisplay.c:100: PulseLcdEnable();							// Enable Pulse
	...
	lcall	_PulseLcdEnable
	pop	ar7
;	LcdDisplay.c:102: PORT_LCD &= 0x0F;        				// Clear old LCD Data (Bit[7..4])
	anl	_P0,#0x0f
;	LcdDisplay.c:103: PORT_LCD |= (i << 4) & 0xF0;			// Strobe Low Nibble Command
	mov	a,r7
	swap	a
	anl	a,#(0xf0&0xf0)
	orl	_P0,a
;	LcdDisplay.c:104: PulseLcdEnable();							// Enable Pulse
	lcall	_PulseLcdEnable
...
;	assignBit
	setb	_P0_3
;	LcdDisplay.c:115: PORT_LCD_RW  = 0;						// Write Select
;	assignBit
	clr	_P0_2
...
;	LcdDisplay.c:119: PulseLcdEnable();							// Enable Pulse  
	...
	lcall	_PulseLcdEnable
	pop	ar7
;	LcdDisplay.c:121: PORT_LCD &= 0x0F;    					// Clear old LCD Data (Bit[7..4])
	anl	_P0,#0x0f
;	LcdDisplay.c:122: PORT_LCD |= (i << 4) & 0xF0; 			// Strobe Low Nibble Data
	mov	a,r7
	swap	a
	anl	a,#(0xf0&0xf0)
	orl	_P0,a
;	LcdDisplay.c:123: PulseLcdEnable();							// Enable Pulse
	lcall	_PulseLcdEnable
...
;	-----------------------------------------
_PulseLcdEnable:
;	LcdDisplay.c:134: PORT_LCD_E = 1;   					// Enable ON
;	assignBit
	setb	_P0_1
;	LcdDisplay.c:135: for (i=0;i<500;i++);
	mov	r6,#0xf4
	mov	r7,#0x01
00104$:
	dec	r6
...
00116$:
	...
	orl	a,r7
	jnz	00104$
;	LcdDisplay.c:136: PORT_LCD_E = 0;   					// Enable OFF 
;	assignBit
	clr	_P0_1
;	LcdDisplay.c:137: }
	ret
...
;	-----------------------------------------
_WaitLcdReady:
;	LcdDisplay.c:146: PORT_LCD_RS  = 0;			 			// Instruction Select
;	assignBit
	clr	_P0_3
;	LcdDisplay.c:147: PORT_LCD_RW  = 1; 					// Read Direction
;	assignBit
	setb	_P0_2
;	LcdDisplay.c:148: PORT_LCD_E = 1;						// Start Read Busy
;	assignBit
	setb	_P0_1
...
;	LcdDisplay.c:151: if(busy_status & 0x80)   				// Read & Check Busy Flag
	movx	a,@dptr
	jnb	acc.7,00102$
;	LcdDisplay.c:153: PORT_LCD_E = 0;       				// Disable Read
;	assignBit
	clr	_P0_1
;	LcdDisplay.c:154: PORT_LCD_RW = 0;					// Default = Write Direction
;	assignBit
	clr	_P0_2
...
;	assignBit
	clr	_P0_1
;	LcdDisplay.c:160: PORT_LCD_RW = 0;					// Default = Write Direction
;	assignBit
	clr	_P0_2
...
;------------------------------------------------------------
;	LcdDisplay.c:167: void ClearDisplay()
;	-----------------------------------------
...
;	LcdDisplay.c:295: for (row = 0; row < ROW_COUNT; ++row)
	...
	mov	r7,#0x00
00108$:
;	LcdDisplay.c:297: SetCursorPos(rowStartAddresses[row]);
	mov	a,r6
	add	a,r6
	mov	r4,a
	mov	a,r7
